# execute.py
"""Franz action executor -- extracts calls, runs them, captures screenshot.

Requires Python 3.13+. Uses only ASCII characters.
No feedback, no stagnation detection, no hash persistence.
"""
...
# Strips PART N -- prefix (with optional "Actions" word) from lines
...
_SAFE_BUILTINS: Final[dict[str, object]] = {
    n: getattr(builtins, n) for n in _SAFE_NAMES
}
...
def _clean_line(line: str) -> str:
    """Strip PART prefixes and legacy world markers."""
    cleaned = line.strip()
    # Strip PART N -- prefix
    cleaned = _PART_PREFIX_RE.sub("", cleaned).strip()
    # Strip legacy world markers
...
def _is_valid_call(text: str) -> bool:
    """Return True when *text* is a single function-call expression."""
    if not text:
        return False
    try:
...
def _call_func_name(text: str) -> str | None:
    """Extract the function name from a call expression, or None."""
    try:
        tree = ast.parse(text, mode="eval")
    except SyntaxError:
        return None
...
def _extract_calls(
    ...
    """Parse the raw story text and return (valid_calls, malformed_errors)."""
    fenced = _FENCE_RE.findall(raw)
    sources: list[str] = (
        ["\n".join(block.strip() for block in fenced)] if fenced else []
    ) + [raw]
        ...
        for line in src.splitlines():
            ...
            if not _is_valid_call(cleaned):
                if re.match(r"^[a-zA-Z_]\w*\s*\(", cleaned):
                    malformed.append(
                        f"UnrecognizedCall: '{cleaned}'"
                        " -- write bare calls like click(x,y)"
                    )
                continue
            ...
            if name in allowed:
                ...
                malformed.append(
                    f"UnknownTool: '{name}' is not an available action"
                )
...
def main() -> None:
    ...
    rd: Path
    if is_debug or not run_dir_str:
        rd = Path(tempfile.mkdtemp(prefix="franz_debug_"))
    else:
        rd = Path(run_dir_str)

    # Optional crop configuration
    crop: dict[str, int] | None = None
    crop_path = rd.parent / "crop.json" if is_debug else rd / "crop.json"
    cd = _load_json(crop_path)
    if isinstance(cd, dict) and all(k in cd for k in ("x1", "y1", "x2", "y2")):
        crop = cd  # type: ignore[assignment]

    # Allowed tools
    tools_path = (
        rd.parent / "allowed_tools.json" if is_debug else rd / "allowed_tools.json"
    )
    ...
    tools.configure(
        physical=bool(_cfg.PHYSICAL_EXECUTION), run_dir=str(rd), crop=crop
    )
    ...
    executed: list[str] = tools.get_results()
